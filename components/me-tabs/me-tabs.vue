		...
		<scroll-view v-if="tabs.length" :id="viewId" :scroll-left="scrollLeft" scroll-x scroll-with-animation
			...
			<view class="tabs-item" :class="{'tabs-flex':!isScroll, 'tabs-scroll':isScroll}">
				...
				<view class="tab-item" :style="{
						...
						color: value===i ? selectColor : txtColor,
					}" v-for="(tab, i) in tabs" :class="{'active': value===i}" :key="i" @click="tabClick(i, tab.val)">
					{{getTabName(tab)}}
				</view>
				<!-- 下划线 -->
				<view class="tabs-line" :style="{left:lineLeft, background: lineColor}"></view>
			</view>
		</scroll-view>
	</view>
</template>
